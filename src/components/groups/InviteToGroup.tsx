...
export default function InviteToGroup({groupId, members, invitations}: {groupId: string, members: User[], invitations: {user_id: string}[]}) {
  ...
  const handleInvite = (id: string) => {
    api.post("group-invitations/send-invitation", {
      user_id: id,
      group_id: groupId
    })
      .then(res => {
                ...
                <>
                  {friends?.filter(friend => {
                    ...
                    <div key={friend.id} className="flex justify-between">
                      ...
                      {invitations.find(user => (user.user_id == friend.id))
                      || sentInvites.includes(friend.id) ? (
                        <Button disabled>Pendiente</Button>
                      ): (
                        <Button color="primary" onPress={() => handleInvite(friend.id)}>Invitar</Button>
                      )}
                    </div>
                  ))}
                </>
