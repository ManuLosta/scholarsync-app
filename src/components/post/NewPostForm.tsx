...
export default function NewPostForm() {
  const { handleSubmit, control, formState: { errors } } = useForm<InputType>({
  ...
  const handleGroupChange = (event) => {
    ...
    if (event.target.value)
      navigate(`/${event.target.value}/new-post`)
  ...
  const onSubmit: SubmitHandler<InputType> = (data) => {
    ...
    if (data.files) {
      ...
      api.post("questions/publish-no-doc-question", {
        title: data.title,
        content: data.body,
        groupId: groupId,
        authorId: user?.id
  ...
  return groups && (
    ...
    >
      <Select
        ...
        renderValue={(items) => {
          return items.map(item => (
            <User key={item.data?.id} avatarProps={{color: "primary"}} className="m-2" name={item.data?.title} />
          ));
        }
        }
      >
        {(group) => (
          <SelectItem key={group.id} textValue={group.id}>
            <User avatarProps={{ color: "primary" }} name={group.title} description={group.description} />
          </SelectItem>
        )}
      </Select>
      <Controller
      ...
      <Controller
        name="body"
        control={control}
        render={({ field: { onChange } }) => <Editor error={errors.body?.message} onChange={onChange} />}
      />
